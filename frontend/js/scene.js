...
const ENABLE_ORANGE_INTERIOR_DEBUG = true;
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  if (expanded) {
    ...
    addDebugBox(
      ...
      0.03,
      new THREE.Vector3(clipCenter.x, clipCenter.y, groundZ + 0.015),
      quat,
      0xff00aa,
      'SpaGroundClipDebug'
    );
...
function ensureOrangeInteriorClipMaterial(material) {
  ...
  material.customProgramCacheKey = () => {
    ...
    return `${prev}|orange-interior-clip-v1`;
  };
...
function setOrangeInteriorClipOnMaterial(material, box) {
  ...
  if (box) {
    ...
    uniforms.orangeInteriorClipMin.value.set(box.minX, box.minY, box.minZ);
    uniforms.orangeInteriorClipMax.value.set(box.maxX, box.maxY, box.maxZ);
  } else {
