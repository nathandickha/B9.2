...
const SPA_WATER_TOP_FINE_ADJUST = 0.07; // adjust top independently (+ up / - down)
...
function updateSpaWalls(spa) {
  ...
  const waterBottomWorld = SPA_WATER_BOTTOM_WORLD;
  const poolWaterTop = SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST;
  const waterHeight = Math.max(0.01, poolWaterTop - waterBottomWorld);
...
export function snapToPool(spa) {
  ...
  if (close === "left") {
    ...
    spa.position.x = Math.abs(x - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD ? centerPos : innerPos;
  ...
  if (close === "front") {
    ...
    spa.position.y = Math.abs(y - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD ? centerPos : innerPos;
